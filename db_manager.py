import threading
from datetime import datetime, timedelta ## sun
from pathlib import Path
from typing import Optional, Union
from fastapi import HTTPException ## sun

from loguru import logger
from sqlalchemy import exc, inspect, text
from sqlmodel import Session, SQLModel, and_, create_engine, select

from ..datamodel import Response, Team, Account ## sun
...
class DatabaseManager:
    ...
    test_sk = 'ksyksyksyksyksyksyksyksyksyksy' ## sun
    ...
    def authenticate(self, account : Account):
        ...
        userInfo = self.get_account(Account, filters={"email": account.email})

        if not userInfo:
            userInfo = self.create_account(account)

        ## detail dict 타입으로 수정 by sun
        elif account.password != userInfo.password:
    ...
    def create_account(self, model: SQLModel):
        """
        Create account & login func()
        """
        model_class = type(model)
        ...
        with Session(self.engine) as session:
            try:
                ...
                session.rollback()
                logger.error("Error while creating " + str(model_class.__name__) + ": " + str(e))
                return None
